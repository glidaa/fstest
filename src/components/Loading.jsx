...
const Loading = (props) => {
  ...
  useEffect(() => {
    const fetchProcess = async () => {
      // Örneğin, burada userActions'dan bir fonksiyon çağırmak yerine doğrudan yorum içine alınmış.
      // const currUser = await dispatch(userActions.handleFetchUser());

      // Kullanıcı durumuna veya rota parametrelerine göre çeşitli kontroller ve işlemler yapılabilir
      // Burada örnek bir kontrol yapısı yerine basitçe bir mesaj güncellemesi ve onFinish çağrısı yapılmıştır.
      setLoadingMsg("Loading completed");
      onFinish();
    };
