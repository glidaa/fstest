...
export const handleSearchUsers =
  (keyword, taskId, purpose = "toAssign") =>
    ...
    if (!app.isOffline) {
      ...
      try {
        const usersData =
          purpose === "toAssign"
            ? (
                await API.execute(searchUserToAssign, {
                  searchQuery: keyword,
                  taskId,
                })
              ).data.searchUserToAssign.items || []
            : (
                await API.execute(searchUserToWatch, {
                  searchQuery: keyword,
                  taskId,
                })
              ).data.searchUserToWatch.items || [];
